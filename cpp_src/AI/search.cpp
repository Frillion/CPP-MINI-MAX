...
int negamax(State& st, int depth, int alpha, int beta){
    ...
    if(depth == 0 && state_outcome != NONE_W)
        return evaluate_state(st, st.get_turn(), state_outcome);
    ...
    for(const move& mv: all_legal_moves){
        ...
        value = std::max(value,-negamax(*successor, depth - 1, -alpha, -beta));
...
std::unique_ptr<move> get_best_move(State& root, int depth){
    ...
    for(const auto& mv : root.get_legal_moves()){
        ...
        int eval = -negamax(*successor, depth, std::numeric_limits<int>::min(), std::numeric_limits<int>::max());
        if(eval > best_value)
...
std::unique_ptr<move> get_best_move_timed(State& root, int max_depth, double time_limit){
    std::unique_ptr<move> best_move = nullptr;
    int depth = 1;
    ...
    while(depth <= max_depth){
        ...
        std::unique_ptr<move> mv = get_best_move(root, depth);
        if(mv)
